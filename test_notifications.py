...
def test_notification_system():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✓ Single notification created (ID: {result['id']})")
        else:
        ...
        if response.status_code == 200:
            result = response.json()
            print(f"✓ Bulk notification sent: {result['message']}")
        else:
            print(f"✗ Failed to send bulk notification: {response.text}")
            return
        ...
        if response.status_code == 200:
            result = response.json()
            print(f"✓ Unread notifications: {result['unread_count']}")
        else:
        ...
        print("=" * 50)
        print("\nSummary:")
        print("✓ Backend API notifications working")
        print("✓ Single user notifications working")
        print("✓ Bulk notifications working")
        print("✓ Notification retrieval working")
        print("✓ Unread count working")
        print("✓ Mark as read working")
        print("\nNext Steps:")
